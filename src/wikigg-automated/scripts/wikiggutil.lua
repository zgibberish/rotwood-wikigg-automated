...
-- text that we prepend to any generated wikitext content
-- to let other wiki writers know
wikiggutil.Const.INFO = [[<!--
Generated by rotwood wikiggutil script (https://github.com/zgibberish/rotwood-wikigg-automated)
(gibberish) PLEASE READ!!!: Any manual edits to the content below might get overwritten,
if you find any issues or want to adjust something, please let me know!
-->]]
...
function wikiggutil.Data.GetAnomalyDefs()
    ...
    local all_defs = Weather.Defs
    all_defs = lume.filter(all_defs, function(def)
        -- workaround to filter out wip/hidden anomalies
        return def.icon ~= "images/ui_ftf_weather_icons1/temp_icon.tex"
    end)
